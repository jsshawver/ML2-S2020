...
We begin with this portion of code is from 9.6.2, which is required for section 9.6.3.
...
In order to implement ROC curves in R, we must first import the ROCR package. Additionally, we must write a short function to plot an ROC curve given the following: the vector 'pred' containing a numerical score for each observation and the vector 'truth' containing the class label for each observation.
...
These predictions seem to be accurate. The increase in gamma made the fit more flexible and increased accuracy.
...
When we compute the ROC curves on the test data, the less flexible model with gamma = 2 is more accurate.
...
This problem uses the OJ data set the ISLR pacakge. A seed is set so that the results can be repeated.
